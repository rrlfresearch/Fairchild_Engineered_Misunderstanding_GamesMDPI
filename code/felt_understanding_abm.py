...
This module implements:
  ...
  - Validation suite reproducing Layer A predictions in the appropriate limit
...
class Params:
    ...
    Defaults match the Atomausstieg calibration in spec Section 6.
    """
    # Population
    N_A: int = 500
    N_B: int = 500
    ...
    # Payoffs and costs (see spec Sec 2.4-2.5)
...
def init_population(p: Params, G: nx.Graph,
    ...
    recognition-capable). Default 0.8 represents an established cooperative
...
def update_beliefs(pop: Population, actions: np.ndarray, observed: dict,
    ...
    Receiver's mental model (per spec Section 3.3): assume the opposing
...
def layer_a_threshold(p: Params, prior_p: Optional[float] = None) -> float:
