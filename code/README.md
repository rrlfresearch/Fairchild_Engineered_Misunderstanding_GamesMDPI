...
This repository contains the agent-based simulation (the manuscript's Computational Model), the validation suite, and the scripts that generate Figures 2 through 6, the scale-free robustness check (Figure 5R), and the two supplementary figures (S1, S2).
...
python fig_S1_sensitivity.py  # regenerates Supplementary Figure S1 (sensitivity)
python fig_S2_initial_belief.py   # regenerates Supplementary Figure S2 (bistability)
```

Output note: the figure-generation scripts currently set `FIG_DIR` near the top of each file. To reproduce the submitted figures on another machine, either update `FIG_DIR` to a writable local folder before running the scripts, or create the directory `/home/RF/work/abm/figures`. After that, install the dependencies with `pip install -r requirements.txt` and run the desired script from the repository root.
...
| `fig_S1_sensitivity.py` | Figure S1: intervention-ranking robustness across (p, c_R, u_R) |
| `fig_S2_initial_belief.py` | Figure S2: bistability of the long-run state in the initial belief μ(0) |
| `sensitivity_interventions.py` | Console companion to Figure S1 (prints the full sensitivity table) |
| `robustness_initial_belief.py` | Console companion to Figure S2 (prints the initial-belief sweep) |
