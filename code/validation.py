...
Validation suite for Layer C of the felt-understanding ABM.

The qualitative prediction from Layer A: action rate is substantially HIGH
...
def check_layer_a_reduction():
    header("Check 1: Layer A reduction (action rate higher at pi=0 than pi=pi*)")
    base_kwargs = dict(T=200, h=0.85, mean_degree=10, gamma=0.0)
    pi_star = layer_a_threshold(Params(**base_kwargs, seed=11))
...
def check_robustness():
    header("Check 5: Specification robustness (SBM and scale-free agree)")
    rates_sbm, rates_sf = [], []
    for s in range(3):
...
def main():
    print("\n" + "#" * 72)
    print("#  Layer C Validation Suite")
    print("#  Specification: Fairchild_Model_Specification_v0.2.docx Sec 5")
    print("#" * 72)
    results = [check_layer_a_reduction(), check_no_adversary_sustainability(),
